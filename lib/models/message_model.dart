import 'package:chat_ui/models/user_model.dart';

class Message {
  final User sender;
  final String time;
  final String text;
  final bool isLiked;
  final bool unread;

  Message({
    required this.sender, 
    required this.time, 
    required this.text, 
    required this.isLiked, 
    required this.unread
  });
}

// You - Current user
final User currentUser = User(
  id: 0, 
  name: 'Current User', 
  imageUrl: 'assets/images/greg.jpg'
);

//USERS
final User greg = User(
  id: 1, 
  name: 'Greg', 
  imageUrl: 'assets/images/greg.jpg'
);

final User james =
    User(id: 2, name: 'James', imageUrl: 'assets/images/james.jpg');

final User john = User(
  id: 3, 
  name: 'john', 
  imageUrl: 'assets/images/john.jpg'
);

final User olivia = User(
  id: 4, 
  name: 'Olivia', 
  imageUrl: 'assets/images/olivia.jpg'
);

final User sam = User(
  id: 5, 
  name: 'Sam', 
  imageUrl: 'assets/images/sam.jpg'
);

final User sophia = User(
  id: 6, 
  name: 'Sophia', 
  imageUrl: 'assets/images/sophia.jpeg'
);

final User steven = User(
  id: 7, 
  name: 'Steven', 
  imageUrl: 'assets/images/steven.jpg'
);

//FAVORITE CONTACTS
List<User> favorites = [sam, steven, olivia, john, greg];

//EXAMPLE CHATS ON HOME SCREEN
List<Message> chats = [
  Message(
    sender: james,
    time: '5:30 PM',
    text: 'Hey, are you there?',
    isLiked: false,
    unread: true,
  ),
  Message(
    sender: olivia,
    time: '4:30 PM',
    text: 'Greg was absent todaay! Do you know anything about?',
    isLiked: false,
    unread: true,
  ),
  Message(
    sender: john,
    time: '3:30 PM',
    text: 'Hey, man! Tomorrow is my birthday. Do you know?',
    isLiked: false,
    unread: false,
  ),
  Message(
    sender: sophia,
    time: '2:30 PM',
    text: 'John invited me to his party. Will you be there',
    isLiked: false,
    unread: true,
  ),
  Message(
    sender: steven,
    time: '1:30 PM',
    text: 'Good afternoon! Long time no see, man!',
    isLiked: false,
    unread: false,
  ),
  Message(
    sender: sam,
    time: '12:30 PM',
    text: 'Hello! Everything´s ok?',
    isLiked: false,
    unread: false,
  ),
  Message( 
    sender: greg,
    time: '11:30 AM',
    text: 'Man, what´s the news?',
    isLiked: false,
    unread: true,
  ),
];

//EXAMPLE MESSAGES IN CHAT SCREEN
List<Message> messages = [
  Message(
    sender: james,
    time: '3:30 PM',
    text: 'Hey, how´s it going? What did you do?',
    isLiked: false,
    unread: true,
  ),
  Message(
    sender: currentUser, 
    time:' 4:30 PM', 
    text: 'Just walked my dog. She was super duper cute. The best pupper!', 
    isLiked: false, 
    unread: true
  ),
  Message(
    sender: james,
    time: '4:45 PM',
    text: 'How´s the dog?',
    isLiked: false,
    unread: true,
  ),
  Message(
    sender: currentUser,
    time: '3:30 PM',
    text: 'Good, thanks! Tomorrow is my birthday. Do you know?',
    isLiked: true,
    unread: false,
  ),
  Message(
    sender: james,
    time: '4:45 PM',
    text: 'Sure! I can´t loose it anyway!',
    isLiked: true,
    unread: true,
  ),
  Message(
    sender: currentUser,
    time: '1:30 PM',
    text: 'Good! You´ll made my day if you come.',
    isLiked: false,
    unread: true,
  ),
  Message(
    sender: james,
    time: '12:30 PM',
    text: 'Did you already invite Sophia?',
    isLiked: false,
    unread: true,
  ),
  Message( 
    sender: james,
    time: '11:30 AM',
    text: 'Sure! She´ll be there.',
    isLiked: false,
    unread: true,
  ),
];
